...
def iter_edbid_from_cve(cve):
    ...
    with open(pdir + "/files_exploits.csv") as files:
...
def update_db():
    ...
    print("Refreshing exploit-database exploits list...")
    ...
    try:
        csv_file = open(pdir + "/files_exploits.csv","r").read()
        ...
        print("Unable to parse exploits csv file")
...
def _search_cve_aux(cve):
    files = open(pdir + "/files_exploits.csv")
    reader = csv.reader(files)
    #reader.next() #skip header
    next(reader)
